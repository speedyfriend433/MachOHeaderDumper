...
class HeaderGenerator {
    ...
    private var includeIvars: Bool = false
    ...
    func generateHeader(includeIvarsInHeader: Bool = false) async throws -> String {
        ...
        // Forward Declarations (method/prop types)
        try collectForwardDeclarations()
        
        // Forward Declarations (Write)
        ...
        let isEmpty = metadata.protocols.isEmpty && metadata.classes.isEmpty && metadata.categories.isEmpty
        if isEmpty {
            print("  [Debug] HeaderGenerator: No dumpable content found. Adding empty message.")
            output += "\n// No Objective-C interfaces, protocols, or categories found to dump.\n"
        }
        
        print("ℹ️ HeaderGenerator: Finished generation. Final output length: \(output.count)")
        return output
    }
    ...
    private func collectForwardDeclarations() throws {
        for cls in metadata.classes.values {
            ...
            try cls.properties.forEach { try collectTypes(from: $0.attributes) }
    ...
    private func checkAndAddForwardDecl(name: String?, isClass: Bool) {
        guard let name = name, !name.isEmpty, name != "NSObject", name != "NSProxy" else { return }
        if isClass {
            if !knownClasses.contains(name) && !name.hasPrefix("<") /* Avoid <Proto> */ {
    ...
    private func collectTypes(from encoding: String?) throws {
        ...
        if typeEncodingToDecode.contains(":") || typeEncodingToDecode.range(of: #"^[v@#][@:]"#, options: .regularExpression) != nil {
            do {
    ...
    private func collectTypes(fromDecoded decoded: DecodedType) {
        ...
        case .class(let name, let protocols):
            ...
            protocols.forEach { checkAndAddForwardDecl(name: $0, isClass: false) }
    ...
    private func writeProtocolDefinition(_ proto: ObjCProtocol) throws {
            ...
            if !proto.baseProtocols.isEmpty {
                declaration += " <\(proto.baseProtocols.sorted().joined(separator: ", "))>"
            }
            print("    [Debug] HeaderGenerator: Writing protocol '\(proto.name)'")
            writeLine(declaration)

            // Write properties and methods (ensure these are being called)
            try writeProperties(proto.instanceProperties)
            try writeClassProperties(proto.classProperties)
            ...
            if !proto.optionalInstanceMethods.isEmpty || !proto.optionalClassMethods.isEmpty {
                 writeLine("@optional")
    ...
    private func writeMethods(_ methods: [ObjCMethod], isClassMethod: Bool, isOptional: Bool = false, contextName: String) throws {
            ...
            for method in methods.sorted(by: { $0.name < $1.name }) {
                do {
                    ...
                    if !isClassMethod && signature.arguments.count >= 1 {
                        let firstArgType = signature.arguments[0] // Implicit 'self'
                        
                        // FIX: Update pattern matching for instancetype check
                        var returnTypeName: String? = nil
                        if case .object(let name, _) = signature.returnType { returnTypeName = name }
                        else if case .class(let name, _) = signature.returnType { returnTypeName = name } // Handles 'id'
                        
                        var firstArgTypeName: String? = nil
                        if case .object(let name, _) = firstArgType { firstArgTypeName = name }
                        else if case .class(let name, _) = firstArgType { firstArgTypeName = name } // Handles 'id'
                        
                        // If return type is an object of the same class as the first arg (self)
                        if let retName = returnTypeName, let argName = firstArgTypeName, retName == contextName, argName == contextName {
                            returnTypeStr = "instancetype"
                        } else if signature.returnType.isId() && firstArgType.isId() { // 'id method(id self, ...)'
                            returnTypeStr = "instancetype"
                        } else if method.name.hasPrefix("init") && signature.returnType.isId() { // Common init pattern
                            returnTypeStr = "instancetype"
                        }
                    }
                // --- End Instancetype ---
